    ...
    <button className={styles.row} onClick={handleClick} type="button">
      ...
      <div className={styles.content}>
        ...
        <div className={styles.dateTime}>
          <span className={styles.date}>{formatDate(report.createdAt)} {report.modifiedAt ? (
            <>- {t('reports.groupReports.lastUpdated', { date: formatDate(report.modifiedAt), time: formatTime(report.modifiedAt) })}</>
          ) : null}
          </span>
        </div>
      </div>
